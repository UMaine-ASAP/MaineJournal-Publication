...
- (CGRect)getSize:(CGPoint)WidthHeight:(int)width:(int)height{
    if (width>height){
        ...
        myRect = CGRectMake(0, (WidthHeight.y/2)-((height*ratio)/2), WidthHeight.x,height*ratio);
    }
    else {
        float ratio = ((WidthHeight.y)/height);
        myRect = CGRectMake( (WidthHeight.x/2)-((width*ratio)/2), 0, width*ratio, WidthHeight.y);
    }
    return myRect;
}
